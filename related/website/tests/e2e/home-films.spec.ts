...
test('one film at a time; switching stops the old film and wraps', async ({ page }) => {
  ...
  await film.evaluate(async video => { video.muted = true; await video.play(); });
  ...
  expect(await old!.evaluate(video => video.paused)).toBe(true);
  await expect(film).toHaveCount(1);
  expect(await film.evaluate(video => video.paused)).toBe(true);
  await section.getByRole('button', { name: 'Next film' }).click();
